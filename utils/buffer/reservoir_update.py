...
class Reservoir_update(object):
    ...
    def update(self, buffer, x, y, **kwargs):
        # if x, y came in as NumPy arrays, convert to torch tensors
        if isinstance(x, np.ndarray):
            x = torch.from_numpy(x).float().to(buffer.buffer_img.device)
        if isinstance(y, np.ndarray):
            y = torch.from_numpy(y).long().to(buffer.buffer_label.device)
        batch_size = x.size(0)
