...
from agents.mir_utils import get_grad_vector, get_future_step_parameters
#from VAE.loss import calculate_loss
from models.VAE.loss import calculate_loss
...
def retrieve_gen(z_pool, y_pool, model, x_batch, y_batch, params):
    """
    Wrapper so your DGR agent can import `retrieve_gen`.
    Right now it just picks the first `batch_size` latents at random.
    You can swap in your MIR logic later.
    """
    # how many to return
    batch_size = params.replay_batch_size
    
    # simple “random subset” strategy:
    idx = np.random.choice(z_pool.size(0), size=batch_size, replace=False)
    return z_pool[idx], y_pool[idx]
